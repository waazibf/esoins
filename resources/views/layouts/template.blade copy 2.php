  ...
  <body>
    ...
    <script>
        ...
        $("#save-form").click(function(){
            ...
            var serie_number = $("#serie_number").val();observation_montant
        ...
        function setPrice(i, table){
            ...
            // console.log(quantity);
            // console.log(quantity);
            var price_drd = $("#price_"+table+i).text();
            if(!quantity_price){
                // alert(quantity);
                $("#total_"+table+i).text(0);
            }else{
                total = parseFloat(quantity_price) * parseFloat(price_drd);

                if(table == 'product'){
                    for(k=0; k<index_product.length; k++){
                        quantity_account = quantity_product[index_product[k]];
                        price_account = price_product[index_product[k]];
                        total_account = total_account + parseFloat(quantity_account) * parseFloat(price_account);
                        // alert(quantity_account);
                    }
                }else if(table == 'acte'){
                    for(k=0; k<index_acte.length; k++){
                        quantity_account = quantity_acte[index_acte[k]];
                        price_account = price_acte[index_acte[k]];
                        total_account = total_account + parseFloat(quantity_account) * parseFloat(price_account);
                        // alert(quantity_account);
                    }
                }else{
                        for(k=0; k<index_equipement.length; k++){
                        quantity_account = quantity_equipement[index_equipement[k]];
                        price_account = price_equipement[index_equipement[k]];
                        total_account = total_account + parseFloat(quantity_account) * parseFloat(price_account);
                        // alert(quantity_account);
                    }
                }
